...
/* Not-a-window */
static void rectangle(double *x, int n){
  int i;
  for(i=0;i<n;i++)
    x[i]=1.;
}
...
/* sine window */
static void sine(double *x, int n){
  int i;
  float scale = M_PI/n;
...
static void blackmann_harris(double *x, int n){
  int i;
...
static void hanning(double *x, int n){
  int i;
  float scale = 2*M_PI/n;
...
#define TGA 1.e-4
#define TGB 21.6
static void tgauss_deep(double *x, int n){
  int i;
  for(i=0;i<n;i++){
    float f = (i+.5-n/2.)/(n/2.);
...
static void vorbis(double *d, int n){
  int i;
  for(i=0;i<n;i++)
    d[i] = sin(0.5 * M_PI * sin((i+.5)/n * M_PI)*sin((i+.5)/n * M_PI));
}

static float beta(int n, double alpha){
...
static void maxwell1(double *x, int n){
  int i;
  float scale = 2*M_PI/n;
  for(i=0;i<n;i++){
    float i5 = i+.5;
